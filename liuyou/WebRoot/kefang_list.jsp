...
<form name="form1" id="form1" method="post" action="">
    搜索: 酒店：<input name="jiudian" type="text" id="jiudian"/> 客房编号：<input name="kefangbianhao" type="text"
                                                                        id="kefangbianhao"/>类型：
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF"
    ...
    <tr>
        ...
        <td bgcolor='#CCFFFF'>酒店</td>
        <td bgcolor='#CCFFFF'>客房编号</td>
    ...
    <%
        ...
        if (request.getParameter("leixing") == "所有" || request.getParameter("leixing") == null) {
        } else {
            sql = sql + " and leixing like '%" + new String(request.getParameter("leixing").getBytes("8859_1")) + "%'";
        }
        if (request.getParameter("daxiao") == "所有" || request.getParameter("daxiao") == null) {
        } else {
    ...
    <tr>
        ...
        <td width="60" align="center"><a href="kefang_updt.jsp?id=<%=id%>">修改</a> <a
                href="del.jsp?id=<%=id %>&tablename=kefang" onClick="return confirm('真的要删除？')">删除</a></td>
    </tr>
    <%
        }
    %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="kefang_list.jsp?page=1">首页</a>　<a
        href="kefang_list.jsp?page=<%= curpage-1%>">上一页</a>　<A href="kefang_list.jsp?page=<%= curpage+1%>">下一页</A>　<a
        href="kefang_list.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %>
